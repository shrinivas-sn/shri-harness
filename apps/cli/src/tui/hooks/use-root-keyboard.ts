...
export function useRootKeyboard(input: {
	...
	const lastExitCtrlCPressTime = useRef(0);
	...
	useKeyboard((key) => {
		...
		if (
			...
			lastExitCtrlCPressTime.current = 0;
		...
		if (key.ctrl && key.name === "c") {
			if (!input.isDialogOpen && hasInputText) {
				...
				if (now - lastExitCtrlCPressTime.current < CONFIRMATION_TIMEOUT_MS) {
					lastExitCtrlCPressTime.current = 0;
					input.onExit();
				} else {
					lastExitCtrlCPressTime.current = now;
					input.onExitHint?.();
				}
			}
			return;
